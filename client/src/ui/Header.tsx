...
import { ChevronDown, Search, ShoppingBag, Star, X } from "lucide-react";
      ...
      <div className="max-w-screen-xl mx-auto h-20 flex items-center justify-between px-4 lg:px-0">
        ...
        <div className="hidden md:inline-flex max-w-3xl w-full relative ">
          <input
            ...
            value={searchText}
            className="w-full flex-1 rounded-full text-gray-900 text-lg placeholder:text-base placeholder:tracking-wide shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 placeholder:font-normal focus:ring-1 foucs:ring-darkText sm:text-sm px-4 py-2"
          />
          {searchText ? (
            <X
              onClick={() => setSearchText("")}
              className="absolute top-[6px] right-4 text-xl hover:text-red-500 cursor-pointer duration-200"
              size={25}
            />
          ) : (
            <Search className="absolute top-[6px] right-4 text-xl" size={25} />
          )}
        </div>
        {/* Serach product will go here */}
        {searchText && (
          <div className="absolute left-0 top-20 w-full mx-auto max-h-[500px] px-10 py-5 bg-white z-20 overflow-y-scroll text-black shadow-lg shadow-skyText">
            {filteredProducts.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5">
                {filteredProducts?.map((item: ProductProps, index: number) => (
                  <ProductCard
                    key={index}
                    item={item}
        ...
        <Container className="py-2 max-w-4xl flex items-center gap-5 justify-between">
          <Menu>
            <MenuButton className="inline-flex items-center gap-2 rounded-md border border-gray-400 hover:border-white py-1.5 px-3 font-semibold text-gray-300 hover:text-whiteText">
              Select Category <ChevronDown size={18} className="mt-1" />
            </MenuButton>
            <Transition
